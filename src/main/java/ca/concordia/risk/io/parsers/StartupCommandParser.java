 ...
 * <p>
 * Implements parsers for the startup phase commands.
 */
public class StartupCommandParser extends CommandParser {
	...
	protected void registerParserMethods() {
		...
		d_commandParsers.put("gameplayer", this::parseGamePlayerCommand);
		d_commandParsers.put("assigncountries", this::parseAssignCountriesCommand);
	}
	 ...
	 * @return <code>GamePlayerCommand</code> if the command was parsed
	...
	private Command parseGamePlayerCommand(List<String> p_argumentList) {
		...
		try {
			while (!p_argumentList.isEmpty()) {
				...
				switch (l_subcommand) {
				case "-add":
					parseAddPlayerBlock(p_argumentList, l_editPlayersCommand);
					break;
				case "-remove":
					parseRemovePlayerBlock(p_argumentList, l_editPlayersCommand);
					break;
				default:
	...
	/**
	 * Parses one <b>-add</b> block of the <i>"gameplayer"</i> command.
	 * 
	 * @param p_argumentList list of command arguments.
	...
	/**
	 * Parses one <b>-remove</b> block of the <i>"gameplayer"</i> command.
	 * 
	 * @param p_argumentList list of command arguments.
	 * @param p_command      command that the parsed data will be stored into.
	 * @throws ParsingException thrown if the number of arguments received is
	 *                          invalid.
	 */
	private void parseRemovePlayerBlock(List<String> p_argumentList, GamePlayerCommand p_command)
